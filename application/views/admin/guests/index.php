...
<div class="card table-card mb-4">
    <div class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <div class="d-flex gap-2 flex-wrap">
            ...
            <?php if (!empty($can_do['guests_export'])): ?>
                <a href="<?= site_url('admin/guests/export_csv/' . $project->id); ?>" class="btn btn-success btn-sm">Export CSV</a>
            <?php endif; ?>

            <?php if (!empty($can_do['guests_export'])): ?>
                <a href="<?= site_url('admin/guests/export_xlsx/' . $project->id); ?>" class="btn btn-outline-success btn-sm">Export XLSX</a>
            <?php endif; ?>
    ...
    <div class="table-responsive">
        <table class="table table-bordered align-middle mb-0">
            ...
            <tbody>
                <?php foreach ($guests as $i => $row): ?>
                    ...
                    <tr>
                        ...
                        <td style="min-width:260px;">
                            <div class="input-group input-group-sm"><input type="text" readonly class="form-control guest-link" value="<?= html_escape($link); ?>"><button type="button" class="btn btn-outline-secondary" onclick="copySingle(this)">Copy</button></div>
                        </td>
                        <td style="min-width:260px;">
                ...
                <?php endforeach; ?>
                <?php if (empty($guests)): ?><tr>
                        <td colspan="7" class="text-center text-muted py-4">Belum ada tamu. Tambah manual atau import file.</td>
                    </tr><?php endif; ?>
            </tbody>
        </table>
    </div>
...
<script>
    ...
    function copyAllLinks() {
        var links = Array.from(document.querySelectorAll('.guest-link'))
            .map(function(el) {
                return el.value;
            })
            .filter(function(val) {
    ...
    function copyWaMessage(btn) {
        copyText(btn.getAttribute('data-message'));
    }

    function copyBulkWa() {
        var el = document.getElementById('wa-bulk-message');
