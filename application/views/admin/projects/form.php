...
<div class="card table-card"><div class="card-body"><form method="post" enctype="multipart/form-data" action="<?= isset($project) ? site_url('admin/projects/update/'.$project->id) : site_url('admin/projects/store'); ?>"><div class="row g-3">
<div class="col-md-4"><label class="form-label">Order</label><select name="order_id" class="form-select" required><option value="">Pilih order</option><?php foreach ($orders as $o): ?><option value="<?= $o->id; ?>" <?= isset($project) && $project->order_id == $o->id ? 'selected' : ''; ?>><?= html_escape($o->order_no ?: ('#'.$o->id)); ?> - <?= html_escape($o->customer_name ?? ('Customer '.$o->customer_id)); ?></option><?php endforeach; ?></select></div>
<div class="col-md-4"><label class="form-label">Product Type</label><?php $pv = isset($project) ? $project->product_type : 'wedding'; ?><select name="product_type" class="form-select"><?php foreach($product_types as $pt): ?><option value="<?= html_escape($pt->code); ?>" <?= $pv == $pt->code ? 'selected' : ''; ?>><?= html_escape($pt->name); ?></option><?php endforeach; ?></select></div>
<div class="col-md-4"><label class="form-label">PIC / Assigned User</label><select name="assigned_user_id" class="form-select"><option value="">-</option><?php foreach($users as $u): ?><option value="<?= $u->id; ?>" <?= isset($project) && $project->assigned_user_id == $u->id ? 'selected' : ''; ?>><?= html_escape($u->name); ?></option><?php endforeach; ?></select></div>
<div class="col-md-4"><label class="form-label">Template</label><select name="template_id" class="form-select" required><?php foreach ($templates as $t): ?><option value="<?= $t->id; ?>" <?= isset($project) && $project->template_id == $t->id ? 'selected' : ''; ?>><?= html_escape($t->name . ' [' . $t->folder . ']'); ?></option><?php endforeach; ?></select></div>
...
<div class="col-md-4"><label class="form-label">Music URL</label><input type="text" name="music_url" class="form-control" value="<?= isset($project) ? html_escape($project->music_url) : ''; ?>"></div>
...
<div class="col-md-6"><label class="form-label">Lokasi</label><input type="text" name="location_name" class="form-control" value="<?= isset($project) ? html_escape($project->location_name) : ''; ?>"></div>
...
<div class="card table-card mt-4"><div class="card-header bg-white"><strong>Project Gallery</strong></div><div class="card-body">
...
<div class="row g-3">
<?php foreach(($galleries ?? array()) as $g): ?><div class="col-md-3"><div class="border rounded p-2 h-100"><img src="<?= asset_or_url($g->image_path); ?>" class="img-fluid rounded mb-2"><div class="small fw-semibold"><?= html_escape($g->caption); ?></div><div class="text-muted small">Sort: <?= (int)$g->sort_order; ?></div><a href="<?= site_url('admin/projects/delete-gallery/'.$g->id); ?>" class="btn btn-sm btn-outline-danger mt-2" onclick="return confirm('Hapus gallery ini?')">Hapus</a></div></div><?php endforeach; ?>
...
<div class="row g-4 mt-2">
    <div class="col-lg-5">
        <div class="card table-card"><div class="card-header bg-white"><strong>Tambah Timeline Project</strong></div><div class="card-body">
    ...
    <div class="col-lg-7">
        <div class="card table-card"><div class="card-header bg-white"><strong>Timeline Project</strong></div><div class="card-body">
